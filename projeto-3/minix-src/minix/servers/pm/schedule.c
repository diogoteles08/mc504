...
int sched_start_user(endpoint_t ep, struct mproc *rmp)
{
	...
	// } else 
		// if (
			// strcmp(mproc[rmp->mp_parent].mp_name, "sshd")
		// )
	// {
	// Verify if process cannot be put on FCFS
	} else {
		printf("Filho do processo: %s\n", (mproc[rmp->mp_parent]).mp_name);

		if (
			strcmp(mproc[rmp->mp_parent].mp_name, "sh") == 0 ||
			strcmp(mproc[rmp->mp_parent].mp_name, "sshd") == 0 ||
			strcmp(mproc[rmp->mp_parent].mp_name, "inetd") == 0 ||
			strcmp(mproc[rmp->mp_parent].mp_name, "syslogd") == 0 ||
			strcmp(mproc[rmp->mp_parent].mp_name, "fsck") == 0 ||
			strcmp(mproc[rmp->mp_parent].mp_name, "dhcpcd") == 0
		) {
			printf("Filho de programa especial!\n");
			inherit_from = INIT_PROC_NR;
		} else {
